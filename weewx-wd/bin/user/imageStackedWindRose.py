...
class ImageStackedWindRoseGenerator(weewx.reportengine.CachedReportGenerator):
        ...
        for timespan in self.image_dict.sections :
            ...
            for plotname in self.image_dict[timespan].sections :
                ...
                if not self.plotgen_ts:
                    self.plotgen_ts = archivedb.lastGoodStamp()
                    if not self.plotgen_ts:
                ...
                for line_name in self.image_dict[timespan][plotname].sections:
                    ...
                    if label:
                        # Convert to Latin-1
                        self.label = weeutil.weeutil.utf8_to_latin1(label).strip()
                    else:
                        # No explicit label so set label to blank
                        self.label = label
                    ...
                    (time_vec_t_ws,data_windSpeed) = archivedb.getSqlVectors(self.obName, self.plotgen_ts-int(plot_options.get('time_length', 86400))+1, self.plotgen_ts, None, None)
                    (time_vec_t_wd,data_windDir) = archivedb.getSqlVectors(self.dirName, self.plotgen_ts-int(plot_options.get('time_length', 86400))+1, self.plotgen_ts, None, None)
                    # Get units for display on legend
                    self.units = self.skin_dict['Units']['Labels'][data_windSpeed[1]]
                    # Find maximum speed from our data
                    maxSpeed = max(data_windSpeed[0])
                    ...
                    i = 0
                    while i < samples:
                        if data_windDir[0][i] is None:
                            windBin[16][6] +=1
                        else:
                            bin = int((data_windDir[0][i]+11.25)/22.5)
                            if data_windSpeed[0][i] > speedList[0][5]:
                                windBin[bin][6] += 1
                            elif data_windSpeed[0][i] > speedList[0][4]:
                                windBin[bin][5] += 1
                            elif data_windSpeed[0][i] > speedList[0][3]:
                                windBin[bin][4] += 1
                            elif data_windSpeed[0][i] > speedList[0][2]:
                                windBin[bin][3] += 1
                            elif data_windSpeed[0][i] > speedList[0][1]:
                                windBin[bin][2] += 1
                            elif data_windSpeed[0][i] > 0:
                                windBin[bin][1] += 1
                            else:
                                windBin[bin][0] += 1
                        i += 1
                    ...
                    self.plotFont = ImageFont.truetype(self.windrose_font_path, self.windrose_plot_font_size)
                    self.legendFont = ImageFont.truetype(self.windrose_font_path, self.windrose_legend_font_size)
                    self.labelFont = ImageFont.truetype(self.windrose_font_path, self.windrose_label_font_size)
                    # Estimate space requried for the legend
                    textWidth, textHeight = draw.textsize("0 (100%)", font=self.legendFont)
                    ...
                    # pixels. Two diameters are calculted, one based on image
                    ...
                    self.roseMaxDiameter = min(int((self.image_height-2*self.windrose_plot_border)/22.0)*22,int((self.image_width-(2*self.windrose_plot_border+legendWidth))/22.0)*22)
                    ...
                    while a < len(windBin): #loop through each wind rose arm
                        ...
                        if cumRadius > 0:
                            ...
                            while s > 0:
                                ...
                                bbox = (self.originX-pieRadius, self.originY-pieRadius, self.originX+pieRadius, self.originY+pieRadius)
                                # Draw pie slice
                                draw.pieslice(bbox, int(-90+a*22.5-self.windrose_plot_petal_width/2), int(-90+a*22.5+self.windrose_plot_petal_width/2), fill=speedList[1][s], outline='black')
                    ...
                    textWidth, textHeight = draw.textsize(label0, font=self.plotFont)
                    # Size the bound box
                    bbox = (int(self.originX-self.roseMaxDiameter/22), int(self.originY-self.roseMaxDiameter/22), int(self.originX+self.roseMaxDiameter/22), int(self.originY+self.roseMaxDiameter/22))
                    draw.ellipse(bbox, outline='black', fill=speedList[1][0])   # Draw the circle
                    draw.text((int(self.originX-textWidth/2),int(self.originY-textHeight/2)), label0, fill=self.windrose_plot_font_color, font=self.plotFont)   # Display the value
                ...
                ngen += 1
        t2 = time.time()
        syslog.syslog(syslog.LOG_INFO, "imageStackedWindRose: Generated %d images in %.2f seconds" % (ngen, t2 - t1))
...
def WindRoseImageSetup(self):
    ...
    try:
        ...
        image = Image.new("RGB", (self.image_width, self.image_height), self.image_background_box_color)
    return image
...
def WindRosePlotSetup(self, draw):
    ...
    i=5
    while i > 0:
        bbox = (self.originX-bbMinRadius*(i+0.5), self.originY-bbMinRadius*(i+0.5), self.originX+bbMinRadius*(i+0.5), self.originY+bbMinRadius*(i+0.5))
        draw.ellipse(bbox, outline=self.image_background_range_ring_color, fill=self.image_background_circle_color)
        i -= 1
    #Draw vertical centre line
    draw.line([(self.originX,self.originY-self.roseMaxDiameter/2-2),(self.originX,self.originY+self.roseMaxDiameter/2+2)],fill=self.image_background_range_ring_color)
    #Draw horizontal centre line
    draw.line([(self.originX-self.roseMaxDiameter/2-2,self.originY),(self.originX+self.roseMaxDiameter/2+2,self.originY)],fill=self.image_background_range_ring_color)
    #Draw N,S,E,W markers
    textWidth, textHeight = draw.textsize('N', font=self.plotFont)
    draw.text((self.originX-textWidth/2,self.originY-self.roseMaxDiameter/2-1-textHeight), 'N', fill=self.windrose_plot_font_color, font=self.plotFont)
    textWidth, textHeight = draw.textsize('S', font=self.plotFont)
    draw.text((self.originX-textWidth/2,self.originY+self.roseMaxDiameter/2+3), 'S', fill=self.windrose_plot_font_color, font=self.plotFont)
    textWidth, textHeight = draw.textsize('W', font=self.plotFont)
    draw.text((self.originX-self.roseMaxDiameter/2-1-textWidth,self.originY-textHeight/2), 'W', fill=self.windrose_plot_font_color, font=self.plotFont)
    textWidth, textHeight = draw.textsize('E', font=self.plotFont)
    draw.text((self.originX+self.roseMaxDiameter/2+1,self.originY-textHeight/2), 'E', fill=self.windrose_plot_font_color, font=self.plotFont)
    ...
    while i<5:
        ...
        draw.rectangle(((self.originX+(2*i+1)*labelOffsetX-textWidth/2,self.originY+(2*i+1)*labelOffsetY-textHeight/2),(self.originX+(2*i+1)*labelOffsetX+textWidth/2,self.originY+(2*i+1)*labelOffsetY+textHeight/2)), fill=self.image_background_circle_color)
        draw.text((self.originX+(2*i+1)*labelOffsetX-textWidth/2,self.originY+(2*i+1)*labelOffsetY-textHeight/2), speedLabels[i-1], fill=self.windrose_plot_font_color, font=self.plotFont)
        i += 1
...
def LegendSetup(self, draw, speedList, speedBin):
    ...
    while i>0:
        ...
        i -= 1
    textWidth, textHeight = draw.textsize(str(speedList[0][0]), font=self.legendFont)
    # Draw 'calm' or 0 speed label and %
    draw.text((labelX+1.5*self.windrose_legend_bar_width,labelY-textHeight/2-(0.85*self.roseMaxDiameter*self.speedFactor[0])),str(speedList[0][0])+' ('+str(int(round(100.0*speedBin[0]/sum(speedBin),0)))+'%)', fill=self.windrose_legend_font_color, font=self.legendFont)
    textWidth, textHeight = draw.textsize('Calm', font=self.legendFont)
    draw.text((labelX-textWidth-2,labelY-textHeight/2-(0.85*self.roseMaxDiameter*self.speedFactor[0])),'Calm', fill=self.windrose_legend_font_color, font=self.legendFont)
    #draw 'calm' bulb on bottom of stacked bar
    bbox = (labelX-bulbDiameter/2+self.windrose_legend_bar_width/2, labelY-self.windrose_legend_bar_width/6, labelX+bulbDiameter/2+self.windrose_legend_bar_width/2, labelY-self.windrose_legend_bar_width/6+bulbDiameter)
    ...
    draw.text((labelX+self.windrose_legend_bar_width/2-textWidth/2,labelY-5*textHeight/2-(0.85*self.roseMaxDiameter)),titleText, fill=self.windrose_legend_font_color, font=self.legendFont)
    #draw legend units label
    textWidth, textHeight = draw.textsize('('+self.units.strip()+')', font=self.legendFont)
    draw.text((labelX+self.windrose_legend_bar_width/2-textWidth/2,labelY-3*textHeight/2-(0.85*self.roseMaxDiameter)),'('+self.units.strip()+')', fill=self.windrose_legend_font_color, font=self.legendFont)
    #draw plot title (label) if any
    if self.label:
        textWidth, textHeight = draw.textsize(self.label, font=self.labelFont)
        draw.text((self.originX-textWidth/2,textHeight/2),self.label, fill=self.windrose_label_font_color, font=self.labelFont)
    #draw plot timestamp if any
    if self.timeStamp:
        timeStampText = datetime.datetime.fromtimestamp(self.plotgen_ts).strftime(self.timeStamp).strip()
        textWidth, textHeight = draw.textsize(timeStampText, font=self.labelFont)
        if self.timeStampLocation != None:
            ...
            timeStampY = self.image_height-self.windrose_plot_border-textHeight
            timeStampX = self.image_width-self.windrose_plot_border-textWidth
        draw.text((timeStampX,timeStampY),timeStampText, fill=self.windrose_legend_font_color, font=self.legendFont)
