...
#    $Revision: 1867 $
#    $Author: mwall $
#    $Date: 2014-01-08 17:51:30 -0500 (Wed, 08 Jan 2014) $
#
...
# FIXME: how to merge services in service_list?
...
class Extension(object):
    ...
    def verify(self):
        archive = tarfile.open(self.filename, mode='r')
        root = None
        has_install = False
        errors = []
    ...
    def extract(self):
        self.archive.extractall(path=self.tmpdir)
    ...
    def get_layout():
        ...
        return {
            ...
            'EXTRACT_ROOT': self.extdir,
            }

class ExtensionInstaller(object):
    """Base class for weewx extension installers."""
    ...
    def prepend_path(self, path):
        ...
        for d in self.dirs:
            if path.startswith(d):
                return path.replace(d, self.dirs[d])
        return path
    ...
    def install_files():
        ...
        for t in self.data_files:
            ...
            try:
                ...
                pass
            for f in t[1]:
                src = "%s/%s" % (self.layout['EXTRACT_ROOT'], f)
    ...
    def merge_config_options():
        fn = "%s/weewx.conf" % self.layout['CONFIG_ROOT']
...
def do_ext():
    ...
    usage = "%prog --install-extension filename"
    parser = optparse.OptionParser(description=description, usage=usage)
    parser.add_option('--install-extension', dest='ext', type=str,
                      metavar="FILE", help='install extension to weewx')
    parser.add_option('--tmpdir', dest='tmpdir', type=str,
                      metavar="DIR", help='install extension to weewx')
    (options, _args) = parser.parse_args()
    ext = Extension(options.ext, options.tmpdir)
    ext.verify()
    ext.extract()
    ext.load()
    ext.install()
